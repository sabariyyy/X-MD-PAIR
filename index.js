            ...
            sparky.ev.on("connection.update", async (s) => {
                ...
                if (connection == "open") {
                    ...
                    if (fs.existsSync(credsPath)) {
                        ...
                        const sessionsparky = fs.readFileSync(credsPath, 'utf8');
                        
                        // Encode to base64 (session ID)
                        const encodedSession = Buffer.from(sessionsparky).toString('base64');
                        
                        // Send session ID in chat
                        await sparky.sendMessage(sparky.user.id, { 
                            text: `*Your Session ID:*\n\`\`\`${encodedSession}\`\`\`\n\n*Instructions:*\nCopy this session ID and save it. You can use it to restore your session later.`
                        });
                        ...
                        await sparky.sendMessage(sparky.user.id, { text: text });
                    }
